...
= ImageBase64
REST API to return an input image as base64. Permission: task_visualization. Path: ../API/extension/image64

The project **Image64** is a Bonita REST API extension for **Bonita 7.12.2** version  **community** written in `groovy`
...
[source,bash]
----
git clone https://your.github.com/Image64.git
cd Image64/
mvn package
----

The build should produce a zip archive under the `target/` folder named `Image64-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `Image64-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
